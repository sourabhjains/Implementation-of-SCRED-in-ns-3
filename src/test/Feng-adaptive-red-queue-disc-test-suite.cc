...
// Tests to verify the working of *automatically set* parameters in SCRED
...
AutoRedQueueDiscTestCase::AutoRedQueueDiscTestCase ()
  : TestCase ("Sanity check on automatically set parameters of SCRED")
{
}
...
// Tests to verify the working of *adaptive* parameter in SCRED
class FengAdaptiveRedQueueDiscTestCase : public TestCase
{
public:
  FengAdaptiveRedQueueDiscTestCase ();
  virtual void DoRun (void);
private:
  void RunFengAdaptiveRedDiscTest (StringValue mode);
};

FengAdaptiveRedQueueDiscTestCase::FengAdaptiveRedQueueDiscTestCase ()
  : TestCase ("Sanity check on adaptive parameter of SCRED")
{
}

void
FengAdaptiveRedQueueDiscTestCase::RunFengAdaptiveRedDiscTest (StringValue mode)
{
  ...
  if (queue->GetMode () == Queue::QUEUE_MODE_PACKETS)
    {
      NS_TEST_EXPECT_MSG_EQ (totalRxBytesCounter, 2574500, "Total received bytes should be 2574500");
    }
  else if (queue->GetMode () == Queue::QUEUE_MODE_BYTES)
    {
      NS_TEST_EXPECT_MSG_EQ (totalRxBytesCounter, 2438500, "Total received bytes should be 2438500");
    }
...
void
FengAdaptiveRedQueueDiscTestCase::DoRun (void)
{
  RunFengAdaptiveRedDiscTest (StringValue ("QUEUE_MODE_PACKETS"));
  RunFengAdaptiveRedDiscTest (StringValue ("QUEUE_MODE_BYTES"));
  Simulator::Destroy ();
}

static class ScredQueueDiscTestSuite : public TestSuite
{
public:
  ScredQueueDiscTestSuite ()
    : TestSuite ("Feng-adaptive-red-queue-disc", UNIT)
  {
    AddTestCase (new AutoRedQueueDiscTestCase (), TestCase::QUICK);         // Tests for automatically set parameters of SCRED
    AddTestCase (new FengAdaptiveRedQueueDiscTestCase (), TestCase::QUICK);     // Tests for Feng's adaptive parameter of SCRED
  }
} g_scredQueueDiscTestSuite;
